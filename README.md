...
### ADR 8: Version-Controlled Database Schema Migrations with Alembic
- **Context:** `Base.metadata.create_all()` is brittle in production: it cannot alter existing columns, add constraints safely, provide rollback capabilities, or track historical schema versions.
- **Decision:** Integrated **Alembic** for linear, version-controlled migrations (`alembic/versions/`). Configured an idempotent migration runner in the FastAPI lifespan startup hook with brownfield auto-stamping and resilient fallback, eliminating manual CLI steps while supporting full rollback and branch inspection.
- **Trade-off:** Requires generating migration scripts for future schema changes, but guarantees reproducible database states across local, staging, and production environments.
...
- Test suite is split into modular Pytest modules (`test_auth.py`, `test_candidates.py`, `test_scores.py`, `test_idempotency.py`, `test_rate_limiter.py`, `test_cache.py`, `test_migrations.py`) covering registration role hardcoding, password strength, score isolation, token blacklisting, soft delete, idempotency replays, rate limiting, Redis caching, and schema migration lifecycles.
...
The application uses **Alembic** for linear, version-controlled database schema migrations. Naive `create_all()` is removed from the production path to ensure safe schema upgrades, rollback support, and zero data loss.

### Migration Lifecycle & Auto-Runner

```mermaid
sequenceDiagram
    autonumber
    actor Dev as Developer / Docker Lifespan
    participant Main as FastAPI Lifespan Hook
    participant Alembic as Alembic Migration Engine
    participant DB as SQLite / Database
    participant Seed as Seed Database Hook

    Dev->>Main: Container Startup / docker compose up
    Main->>Alembic: run_migrations() -> upgrade(alembic_cfg, "head")
    Alembic->>DB: Inspect alembic_version table
    alt Legacy unversioned DB detected
        Alembic->>DB: Brownfield auto-stamp to baseline (001_initial_schema)
    else Pending revisions exist
        Alembic->>DB: Apply pending migration revisions
        Alembic->>DB: Update alembic_version to head
    else Already at head
        Alembic->>DB: No-op (up to date)
    end
    Main->>Seed: seed_database(db)
    Main->>Dev: API Ready with current schema
```

### Alembic CLI Reference Commands
Developers and DevOps engineers can manage schema versions via standard CLI commands inside the Docker container:
...
# 4. Rollback exactly 1 revision
docker compose run --rm backend alembic downgrade -1

# 5. Rollback completely to base
docker compose run --rm backend alembic downgrade base

# 6. Generate a new autogenerated migration after modifying models
docker compose run --rm backend alembic revision --autogenerate -m "add_interview_notes_column"
```
